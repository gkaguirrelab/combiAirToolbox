...
// intensity, using a device custom-built by Vincent Lau. The Arduino board
// is an Arduino nano, with a baud rate of 115200.
//
// Note that the ATmega32p (old bootloader) must be selected from the
// Arduino IDE menu:  Tools --> Processor
// 
//
...
const int interLineIntervalMs = 500;  // Brief delay between addressing the control lines
...
void loop() {
  ...
  if (modulationState) {
    ...
    if ((currentTime - lastTrialStartTime) > trialDurMicroSecs) {
      ...
      // Deliver a puff
      deliverPuff(stimPressuresPSI[stimIdxSeq[trialIdx]],stimDursMs[stimIdxSeq[trialIdx]]);
...
void getConfig() {
  ...
  if (strncmp(inputString, "SP", 2) == 0) {
    ...
    clearInputString();
    waitForNewString();
    int nTypes = atoi(inputString);
    Serial.println(nTypes);
    clearInputString();
...
void getDirect() {
  ...
  if (strncmp(inputString, "PP", 2) == 0) {
    Serial.println("PP:");
    clearInputString();
    deliverPuff(stimPressurePSIDirect,stimDurMsDirect);
  }
...
void deliverPuff(float puffPressurePSI, int puffDurationMs) {
  // Set the stimulus pressure
  setPressure(puffPressurePSI,pressureSetTimeMs);
  // Now we open the valve on the "controlLineBlue", causing air
  // to flow to the subject
  digitalWrite(controlLineBlue, HIGH);
  // Wait for the puffDuration
  delay(puffDurationMs);
  // Close the line
...
void clearPiston() {
  ...
  // and we again wait an extra long time
